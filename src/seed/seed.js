...
const DELAY_MS = Number.parseInt(process.env.DELAY_MS ?? process.env.DELAY ?? "250", 10);
...
async function fetchWithRetry(url, options = {}, retries = 3, backoffMs = 500) {
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      console.warn(`⚠️  Fetch failed (${err.cause?.code || err.code}), retry ${attempt + 1}/${retries} in ${backoffMs}ms...`);
...
async function insertCommentary(matchId, entry) {
  ...
  const response = await fetchWithRetry(`${API_URL}/matches/${matchId}/commentary`, {
    method: "POST",
    headers: { "content-type": "application/json" },
